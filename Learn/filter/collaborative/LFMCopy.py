...
def getUserNegativeItem(frame, userID):
    ...
    :return: 负反馈物品
    '''
    userItemlist = set(frame[frame['UserID'] == userID]['MovieID'].values)  # 用户评分过的物品
...
def recommend2(frame, userID, p, q, TopN=10):
    '''
    推荐TopN个物品给目标用户
    :param frame: 源数据
    :param userID: 目标用户
    ...
    :return: 推荐物品
    '''
    userItemlist = list(set(frame['MovieID']))
    predictList = [lfmPredict(p, q, userID, itemID) for itemID in userItemlist]
    series = pd.Series(predictList, index=userItemlist)
    series = series.sort_values(ascending=False)[:TopN]
    return series
